...
def make_predicts(model, data_loader, device):
    y_preds = []
    model.eval()
    ...
    with torch.inference_mode():
        for X, y in tqdm(data_loader, desc="Making predictions"):
            ...
            y_pred = np.round(torch.sigmoid(y_pred_logits).cpu().detach().numpy())
            # Put predictions on CPU for evaluation
            y_preds.append(y_pred)

    return torch.tensor(y_preds).squeeze()
